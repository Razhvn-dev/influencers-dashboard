...
function applyCreatorAutomation(data) {
  ...
  const ambassadorLevel =
    data.ambassador_level != null && String(data.ambassador_level).trim() !== ''
      ? String(data.ambassador_level).trim()
      : null;
...
function mergeProfileFields(existing, body) {
  return {
    ...
    ambassador_level:
      body.ambassador_level !== undefined
        ? body.ambassador_level
        : existing.ambassador_level,
  };
}
...
function normalizeProfilePayload(profile) {
  return applyCreatorAutomation({
    ...
    ambassador_level: profile.ambassador_level ?? null,
  });
}
