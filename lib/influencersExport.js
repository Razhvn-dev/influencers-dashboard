const { computeAmbassadorLevelFromFollowers } = require('./ambassadorLevel');

function displayAmbassadorLevel(record) {
  return computeAmbassadorLevelFromFollowers(record?.total_followers);
}
  ...
  const baseColumns = [
    {
      ...
      value: (record) => record.business_name,
      kind: 'text',
    },
    { label: 'First Name', value: (record) => record.first_name, kind: 'text' },
    { label: 'Last Name', value: (record) => record.last_name, kind: 'text' },
    { label: 'Name', value: (record) => record.name, kind: 'text' },
    { label: 'Status', value: (record) => record.status, kind: 'status' },
    {
      label: 'Ambassador Level',
      value: (record) => displayAmbassadorLevel(record),
      kind: 'text',
    },
    ...
    {
      label: 'Followers Last Verified',
      value: (record) => formatDateTime(record.followers_last_verified_at),
      kind: 'datetime',
    },
    {
      label: 'Followers Verified By',
      value: (record) => record.followers_verified_by,
      kind: 'text',
    },
